...
다른 하나는 Blueprint (BP)를 작성하여 pak파일로 Pal 폴더에 넣는 것이다. 

두 가지 방법 모두 장단점이 있다. 
Lua는 스크립트 언어이기 때문에 코딩에 익숙해야 하지만, 
빠르게 프로토타입을 만들고 테스트 할 수 있으며 BP보다 좀 더 많은 것을 코딩으로 해결할 수 있다. 
그에 반해 BP는 코딩 없이 블럭과 선으로 마치 레고처럼 결과물을 만들 수 있어 접근성이 좋지만 일일히 pak 파일을 만드는 것은 상당히 귀찮다. 
특히 Unreal Engine의 변수 호출 방식과 수많은 함수에 익숙하지 않은 초기에는 코드 실행이 잘 되는지 계속 부딪혀가며 확인해야 하기 때문에 코드로 일단 돌려보는 것이 좋다고 생각한다.  

그리고 무엇보다 이 프로젝트에서는 OS 환경과 연결하여 csv 파일을 기록할 것이기 때문에 Lua를 통한 Mod 파일을 작성하는 방법을 선택한다. 
...
[여기](https://github.com/UE4SS-RE/RE-UE4SS/releases)서 최신 release 버전에서 
