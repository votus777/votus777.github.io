...
다른 비유를 하자면 소개팅을 받기로 했는데 웬일인지 동시에 서로 다른 사람에게서 3명을 소개 받게 되었다.   
A,B,C 이렇게 있는데 일단 프사를 보니 A가 가장 괜찮아 보이고 맘에 든다.     
그래서 바로 A를 선택하고 소개팅에 나가고,    
이후의 소개팅에서도 프로필 사진이 가장 멋있어 보이는 사람을 선택하는 것이 Exploit이다.    
하지만 그렇게 기대감을 잔뜩 안고 카페에 나갔는데 항상 별 다른 일 없이 소개팅이 평범하게 끝난다.    
여기에 항상 만족하고 안주할 수도 있다.

하지만 당장 Exploit을 하지 않고 B를 선택 했을때,   
첫 호감은 없었고 얼굴은 평범하지만 대화가 계속 이어지고 자신과 성격이 아주 잘 맞는 사람이었다면 어땠을까.         
오히려 사진을 보고 선택한 A보다 B가 호감이 생겨 애프터까지 갈 수 있었을 것이다.      
 
충분한 Explore가 따라주지 않았기에 놓친 reward다.       
보상에 대한 기대값은 적었지만 실제 전체 보상은 보다 커질 수 있다.            
앞으로도 소개팅이 동시에 여럿 들어올 때 계속 얼굴만 보고 소개팅을 나가면 이런 기회를 놓칠 것이다.   

하지만 그렇다고 항상 explore 하는 것은 굉장히 많은 시간과 정신, 물질적 비용이 발생할 것이다.           
기대를 하지 않고 나갔는데 실제로 기대 이하면   
그 때 발생한 비용은 고스란히 낭비가 되어 버리기 때문이다.      

그래서 이런 상황에서 어떻게 행동할 것인지가 이 trade-off 관계의 핵심이 된다.    
(하지만 실제 Multi-armed Bandit problem에서는 조금 더 복잡한 조건들이 추가된다).       
